...
public class Mage_Soldier extends Piece {
    int hp;
    public Mage_Soldier(int player, int x, int y, int hp){
    ...
    public List<Movement> getMovements(Game state){
        ...
        for(Pares possible_moves : visited){
            // Checks if there're pieces in its path, if so the path is invalid, because pieces cannot go through other pieces.
                ...
                moves.add(new Movement(Movement.moveCommand(this.x,this.y,possible_moves.x,possible_moves.y),cpy)); //Almacena el movimiento y el grid despues del mov en m
        }
        return moves;
    }
    ...
    // Clone Mage_Soldier so that the clone is a Mage_Soldier instance
    @Override
    public Piece clonePiece(){
        Mage_Soldier other = new Mage_Soldier(player,x,y,hp);
        return other;
    }
    void update(){
        this.hp -= 1;
